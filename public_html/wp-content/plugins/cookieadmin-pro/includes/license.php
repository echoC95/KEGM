...
class License{
	...
	static function cookieadmin_pro_license(){
		...
		if(!is_array($lic_resp)){
			$cookieadmin_error = __('The response was malformed', 'cookieadmin-pro').' '.esc_html($lic_resp);
			return;
		}
				...
				<table class="wp-list-table fixed striped users cookieadmin-pro-license-table" cellspacing="1" border="0" width="100%" cellpadding="10" align="center">
					<tbody>
						<tr>				
							...
							<td><?php
								echo esc_html(COOKIEADMIN_PRO_VERSION).' ('.esc_html__('Pro Version', 'cookieadmin-pro').')';
							?>
							</td>
						</tr>
						<tr>			
							...
							<td align="left">
								...
								<?php if(!empty($cookieadmin['license']['license'])){
										...
										'.($cookieadmin['license']['expires'] <= gmdate('Ymd') ? esc_html__('License Expires', 'cookieadmin-pro').' : <span style="color:red;">'.esc_attr($expires).'</span>' : (empty($cookieadmin['license']['has_plid']) ? esc_html__('License Expires', 'cookieadmin-pro').' : '.esc_html($expires) : '')).'
										</div>';
